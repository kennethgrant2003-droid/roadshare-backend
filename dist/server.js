...
/* =========================================================
   EXPRESS
========================================================= */
app.use("/api/stripe/webhook", express_1.default.raw({
...
io.on("connection", (socket) => {
    console.log("[RoadShare Socket] connected:", socket.id);
    /* =======================================================
       USER JOIN
    ======================================================= */
    socket.on("user:join", (payload, callback) => {
         ...
         */
        if (role === "helper") {
            const jobs = Array.from(activeJobs.values()).filter((job) => job.status === "searching");
            jobs.forEach((job) => {
    ...
    /* =======================================================
       JOB ROOM JOIN / LEAVE
    ======================================================= */
    socket.on("job:join", (payload, callback) => {
        const jobId = payload?.jobId
            ? String(payload.jobId)
            : "";
        if (!jobId) {
        ...
        socket.join(`job:${jobId}`);
        console.log("[RoadShare Socket] joined room:", `job:${jobId}`);
        callback?.({
            ok: true,
            jobId,
        });
    });
    socket.on("job:leave", (payload) => {
        const jobId = payload?.jobId
            ? String(payload.jobId)
            : "";
        if (!jobId)
        ...
        console.log("[RoadShare Socket] left room:", `job:${jobId}`);
    });
    /* =======================================================
       CREATE JOB
    ======================================================= */
    socket.on("job:create", (payload, callback) => {
        ...
        const quoteCents = Number(payload?.quoteCents);
        const customerLocation = payload?.location ||
        ...
        const job = {
            ...
            paymentStatus: payload?.paymentStatus ||
                "paid",
            quoteCents: Number.isFinite(quoteCents) &&
                quoteCents >= 50
        ...
        socket.join(`job:${jobId}`);
        console.log("[RoadShare Socket] job:create", job);
        /*
    ...
    socket.on("job:accept", (payload, callback) => {
        ...
        if (!existing) {
            callback?.({
                ok: false,
                error: "Job was not found or is no longer available.",
            });
            return;
        }
        /*
        ...
        if (existing.status !== "searching") {
            callback?.({
                ok: false,
                error: "This RoadShare request has already been accepted.",
            });
            return;
        }
        const acceptedJob = {
            ...
            helperProfile: {
                helperId: payload?.helperId ||
                    socket.id,
                name: payload?.helperName ||
                    "RoadShare Helper",
                phone: payload?.helperPhone ||
                    "",
                vehicle: payload?.helperVehicle ||
                    "",
            },
        ...
        socket.join("helper");
        socket.join(`job:${jobId}`);
        console.log("[RoadShare Socket] job:accept", acceptedJob);
        /*
         ...
         */
        io.to("helper").emit("job:unavailable", {
            jobId,
        });
        callback?.({
            ok: true,
    ...
    socket.on("location:update", (payload) => {
        ...
        const update = {
            jobId,
            helperUserId: payload?.helperUserId ||
                payload?.helperId ||
                undefined,
            latitude,
            longitude,
        ...
        console.log("[RoadShare Socket] location:update", update);
        /*
    ...
    socket.on("job:update_status", (payload, callback) => {
        ...
        activeJobs.set(jobId, job);
        console.log("[RoadShare Socket] job:update_status", job);
        /*
         * One canonical server event:
         * job:status_updated
         */
    ...
    socket.on("disconnect", (reason) => {
        console.log("[RoadShare Socket] disconnected:", socket.id, reason);
    });
});
/* =========================================================
   START SERVER
